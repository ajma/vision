...
public abstract class Controller {
    ...
    protected Result execute(String actionString, String idString,
            Properties params) {
        try {
            ...
            if (paramTypes == null || paramTypes.length == 0) {
                return (Result) action.invoke(this);
            } else {
                if (int.class.equals(paramTypes[0])) {
                    int id = Integer.parseInt(idString);
                    return (Result) action.invoke(this, id);
                } else {
                    Object param = mapParamsToObject(params, paramTypes[0]);
                    return (Result) action.invoke(this, param);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
